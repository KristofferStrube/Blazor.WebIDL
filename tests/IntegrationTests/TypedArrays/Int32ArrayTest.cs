namespace IntegrationTests.TypedArrays;

[TestFixture("Chrome")]
[TestFixture("Firefox")]
[TestFixture("Webkit")]
public class Int32ArrayTest(string browserName) : BlazorTest(browserName)
{
    [Test]
    public async Task CreateAsync_WithNoArguments_Succeeds()
    {
        // Act
        Int32Array array = await Int32Array.CreateAsync(JSRuntime);
    }

    [Test]
    public async Task CreateAsync_WithLength_CreatesArrayWithLength()
    {
        // Act
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 4);

        // Assert
        long length = await array.GetLengthAsync();
        length.Should().Be(4);
    }

    [Test]
    public async Task CreateAsync_WithArrayBuffer_CreatesArrayFromBuffer()
    {
        // Arrange
        await using Int16Array originalArray = await Int16Array.CreateAsync(JSRuntime, 4);
        await originalArray.FillAsync(1, 0, 3);
        await using IArrayBuffer arrayBuffer = await originalArray.GetBufferAsync();

        // Act
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, arrayBuffer);

        // Assert
        int firstElement = await array.AtAsync(0);
        int secondElement = await array.AtAsync(1);
        firstElement.Should().Be(ushort.MaxValue + 2);
        secondElement.Should().Be(1);
    }

    [Test]
    public async Task CreateAsync_WithArrayBufferAndByteOffset_CreatesArrayFromBuffer()
    {
        // Arrange
        await using Int16Array originalArray = await Int16Array.CreateAsync(JSRuntime, 8);
        await originalArray.FillAsync(1);
        await using IArrayBuffer arrayBuffer = await originalArray.GetBufferAsync();

        // Act
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, arrayBuffer, 4);

        // Assert
        int sum = 0;
        for (int i = 0; i < 3; i++)
        {
            sum += await array.AtAsync(i);
        }
        sum.Should().Be((ushort.MaxValue * 3) + (2 * 3));
    }

    [Test]
    public async Task CreateAsync_WithArrayBufferByteOffsetAndLength_CreatesArrayFromBuffer()
    {
        // Arrange
        await using Int16Array originalArray = await Int16Array.CreateAsync(JSRuntime, 8);
        await originalArray.FillAsync(1);
        await using IArrayBuffer arrayBuffer = await originalArray.GetBufferAsync();

        // Act
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, arrayBuffer, 4, 2);

        // Assert
        int result = await array.AtAsync(0);
        result.Should().Be(ushort.MaxValue + 1 + 1);
    }

    [Test]
    public async Task CreateAsync_WithTypedArray_CreatesArrayFromTypedArray()
    {
        // Arrange
        await using Int32Array originalArray = await Int32Array.CreateAsync(JSRuntime, 2);
        await originalArray.FillAsync(1, 0, 1);
        await originalArray.FillAsync(2, 1, 2);

        // Act
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, originalArray);

        // Assert
        int firstElement = await array.AtAsync(0);
        int secondElement = await array.AtAsync(1);
        firstElement.Should().Be(1);
        secondElement.Should().Be(2);
    }

    [Test]
    public async Task GetBufferAsync_GetsBuffer()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);

        // Act
        await using IArrayBuffer buffer = await array.GetBufferAsync();
    }

    [Test]
    public async Task AtAsync_WithPositiveNumber_GetsElementFromStartOfArray()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);
        await array.FillAsync(10);
        await array.FillAsync(20, 5);

        // Act
        int secondElement = await array.AtAsync(1);

        // Assert
        secondElement.Should().Be(10);
    }

    [Test]
    public async Task AtAsync_WithNegativeNumber_GetsElementFromEndOfArray()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);
        await array.FillAsync(10);
        await array.FillAsync(20, 5);

        // Act
        int secondElement = await array.AtAsync(-1);

        // Assert
        secondElement.Should().Be(20);
    }

    [Test]
    public async Task FillAsync_WithNoArguments_FillsEntireArray()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);

        // Act
        await array.FillAsync(10);

        // Assert
        int firstElement = await array.AtAsync(0);
        int lastElement = await array.AtAsync(-1);
        firstElement.Should().Be(10);
        lastElement.Should().Be(10);
    }

    [Test]
    public async Task FillAsync_WithStart_FillsArrayFromStartArgument()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);

        // Act
        await array.FillAsync(10, 5);

        // Assert
        int firstElement = await array.AtAsync(0);
        int lastElement = await array.AtAsync(-1);
        firstElement.Should().Be(0);
        lastElement.Should().Be(10);
    }

    [Test]
    public async Task FillAsync_WithStartAndEnd_FillsArrayFromStartArgumentToEndArgument()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);

        // Act
        await array.FillAsync(10, 1, 3);

        // Assert
        int firstElement = await array.AtAsync(0);
        int secondElement = await array.AtAsync(1);
        int thirdElement = await array.AtAsync(2);
        int fourthElement = await array.AtAsync(3);
        firstElement.Should().Be(0);
        secondElement.Should().Be(10);
        thirdElement.Should().Be(10);
        fourthElement.Should().Be(0);
    }

    [Test]
    public async Task GetLengthAsync_GetsLength()
    {
        // Arrange
        await using Int32Array array = await Int32Array.CreateAsync(JSRuntime, 10);

        // Act
        long length = await array.GetLengthAsync();

        // Assert
        length.Should().Be(10);
    }
}
